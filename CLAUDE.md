...
- **LLM layer** (`app/llm.py`) — manages the chat loop with prompt-based tool calling via OpenRouter (using the OpenAI SDK). The LLM emits `\`\`\`tool` fenced blocks which are parsed and executed in a loop (up to 15 rounds per message).
- **Tool system** (`app/tools.py`) — six tools (list_wiki, read_wiki, write_wiki, read_source, list_sources, search_wiki) that the LLM calls to read/write files. Search uses `qmd` (hybrid BM25/vector) with a substring fallback.
